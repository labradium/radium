...
async function main() {
    ...
    const project = await terminal.group({
        ...
        choosePackageManager: () => terminal.select({
    ...
    try {
        await copyDirectory(`./templates/${project.chooseTemplate}`, `./${project.name}`);
        await updatePackageJson(`./${project.name}`, project.name);
        terminal.note(`${project.chooseTemplate} Template Initialized successfully!`, "Congratulations!");
    }
    catch (err) {
    ...
    terminal.outro(`Problems? ${color.underline(color.cyan("https://github.com/silver-radium/init/issues"))}`);
}
